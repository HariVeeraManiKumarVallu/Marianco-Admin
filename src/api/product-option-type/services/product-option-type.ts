...
export default factories.createCoreService('api::product-option-type.product-option-type', ({ strapi }) => ({
	async getOptionTypes(options: PrintifyOption[]) {
		...
		const newOptionTypes = await Promise.all(
